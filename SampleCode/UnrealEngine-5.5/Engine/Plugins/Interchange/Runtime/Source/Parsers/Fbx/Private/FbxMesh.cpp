...
bool FFbxMesh::GetGlobalJointBindPoseTransform(FFbxParser* Parser, FbxScene* SDKScene, FbxNode* Joint, FbxAMatrix& GlobalBindPoseJointMatrix, TMap<FString, FMatrix>& MeshIdToGlobalBindPoseJointMap, TMap<FString, FMatrix>& MeshIdToGlobalBindPoseReferenceMap, bool& bBadBindPoseMessageDisplay)
{
	...
	bool bGlobalBindPoseJointMatrixSet = false;
		...
		for (int32 GeometryDeformerIndex = 0; GeometryDeformerIndex < GeometryDeformerCount; ++GeometryDeformerIndex)
		{
			...
			for (int32 ClusterIndex = 0; ClusterIndex < ClusterCount; ClusterIndex++)
			{
				...
				if (Joint == Cluster->GetLink())
				{
					...
					if (!bGlobalBindPoseJointMatrixSet)
					{
						//We use the first geo bind pose
						GlobalBindPoseJointMatrix = GlobalBindPoseJointMatrixPerGeo;
						bGlobalBindPoseJointMatrixSet = true;
					}
