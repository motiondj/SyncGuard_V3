...
FAVResult TVideoDecoderVT<TResource>::ReceiveFrame(TResolvableVideoResource<TResource>& InOutResource)
{
    if(IsOpen())
    {
        if(Frames.Peek())
        {
            ...
            static CVMetalTextureCacheRef MetalTextureCache = nullptr;
            if (MetalTextureCache == nullptr)
            {
                id<MTLDevice> Device = (__bridge id<MTLDevice>)GDynamicRHI->RHIGetNativeDevice();
                check(Device);
                
                CVReturn Result = CVMetalTextureCacheCreate(kCFAllocatorDefault, nullptr, Device, nullptr, &MetalTextureCache);
            ...
            CVReturn Result = CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, MetalTextureCache, Frame->ImageBuffer, nullptr, MTLPixelFormatBGRA8Unorm_sRGB, Width, Height, 0, &TextureRef);
            if (Result != kCVReturnSuccess)
            {
