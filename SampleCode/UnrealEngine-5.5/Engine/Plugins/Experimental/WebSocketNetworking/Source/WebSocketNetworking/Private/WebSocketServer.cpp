...
bool FWebSocketServer::Init(uint32 Port, FWebSocketClientConnectedCallBack CallBack, FString InBindAddress)
{
#if USE_LIBWEBSOCKET
	#if !UE_BUILD_SHIPPING
	// setup log level.
	...
	Info.iface = NULL;
	if (!InBindAddress.IsEmpty())
	{
		size_t AddressLen = FCStringAnsi::Strlen(StringCast<ANSICHAR>(*InBindAddress).Get());
		BindAddress = new char[AddressLen + 1];
		FCStringAnsi::Strncpy(BindAddress, StringCast<ANSICHAR>(*InBindAddress).Get(), AddressLen+1);
		Info.iface = BindAddress;
	}
