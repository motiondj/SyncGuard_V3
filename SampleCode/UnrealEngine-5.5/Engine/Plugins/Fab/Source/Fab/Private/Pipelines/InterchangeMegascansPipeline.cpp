...
#define MEGASCAN_MATERIAL_BLEND_MODE_OVERRIDE_KEY		    MEGASCAN_MATERIAL_KEY TEXT(".BlendModeOverride")
...
)
{
	...
	if (MegascanImportType == EMegascanImportType::Plant && MegascanAssetTier > EMegascanImportTier::Raw)
	{
		...
		{
			for (int32 Index = 0, MaxIndex = ImportedMesh->GetNumSourceModels(); Index < MaxIndex; ++Index)
			{
				ImportedMesh->GetSourceModel(Index).ScreenSize = (Index < MaxIndex - 1) ? FMath::Pow(0.75f, Index) : (0.05 * static_cast<int8>(MegascanAssetTier));
			}
		}
	}
...
void UInterchangeMegascansPipeline::SetupTextureParams(UInterchangeTextureFactoryNode* TextureFactoryNode, const TSharedPtr<FJsonObject>& TextureParams)
{
	if (FString CompressionSettings; TextureParams->TryGetStringField(TEXT("compression"), CompressionSettings))
	{
		...
		else if (CompressionSettings == "displacement")
			TextureFactoryNode->SetCustomCompressionSettings(TC_Displacementmap);
	}
...
void UInterchangeMegascansPipeline::SetupMaterial(UInterchangeMaterialInstanceFactoryNode* MaterialInstanceFactoryNode)
{
	if (MegascanImportType == EMegascanImportType::Plant && MegascanAssetTier == EMegascanImportTier::Raw)
	{
		...
		MaterialInstanceFactoryNode->SetAttribute(MEGASCAN_MATERIAL_BLEND_MODE_KEY, static_cast<int>(BLEND_Opaque));
		MaterialInstanceFactoryNode->AddApplyAndFillDelegates<int>(
			MEGASCAN_MATERIAL_BLEND_MODE_KEY,
...
void UInterchangeMegascansPipeline::SetupMaterialParents(UInterchangeMaterialInstanceFactoryNode* MaterialInstanceFactoryNode, const FString& CustomType)
{
	switch (MegascanImportType)
	{
		case EMegascanImportType::Model3D:
		{
			if (CustomType.IsEmpty())
				SetMegascanMaterialType(MaterialInstanceFactoryNode, EMegascanMaterialType::Base3D);
			else if (CustomType == "transmission")
				SetMegascanMaterialType(MaterialInstanceFactoryNode, EMegascanMaterialType::Transmission3D);
			else if (CustomType == "fuzz")
				SetMegascanMaterialType(MaterialInstanceFactoryNode, EMegascanMaterialType::Fuzz3D);
			else if (CustomType == "glass")
				SetMegascanMaterialType(MaterialInstanceFactoryNode, EMegascanMaterialType::Glass3D);
		}
		break;
		case EMegascanImportType::Surface:
		{
			if (CustomType.IsEmpty())
				...
				SetMegascanMaterialType(MaterialInstanceFactoryNode, EMegascanMaterialType::TransmissionSurface);
		}
		break;
		...
		break;
		case EMegascanImportType::Imperfection:
		{
			SetMegascanMaterialType(MaterialInstanceFactoryNode, EMegascanMaterialType::Surface);
		}
		break;
	...
	// if (MegascanImportType == EMegascanImportType::Plant && ImportTier == EMegascanImportTier::Raw)
	// {
	// 	const FString ParameterName = UInterchangeShaderPortsAPI::MakeInputValueKey(TEXT("secondary Animation"));
	// 	MaterialInstanceFactoryNode->AddBooleanAttribute(ParameterName, true);
	// }
}
