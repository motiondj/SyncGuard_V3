...
void FQuixelGltfImporter::ImportGltf3DAsset(const FString& SourcePath, const FString& DestinationPath, const bool bBuildNanite, TFunction<void(const TArray<UObject*>&)> OnDone)
{
	const UInterchangeSourceData* InSourceData = UInterchangeManager::CreateSourceData(SourcePath);
