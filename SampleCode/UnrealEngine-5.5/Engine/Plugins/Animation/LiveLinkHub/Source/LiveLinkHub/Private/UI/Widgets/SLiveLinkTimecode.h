...
private:
	...
	void SendUpdatedTimecodeToEditor(const FLiveLinkHubTimecodeSettings& TimecodeSettings);

	FLiveLinkHubTimecodeSettings MakeTimecodeSettings() const;

	/** Update the local timecode properties from the timecode settings. */
	void UpdateTimecodeSettingsFromSession(const TSharedRef<class ILiveLinkHubSession>& InSession);
	
	/** Do we have timecode source enabled */
	bool bCachedIsTimecodeSource = false;
