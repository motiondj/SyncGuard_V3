...
void UPCGDataBinding::InitializeInputData(const FPCGDataCollection& InComputeGraphElementInputData)
{
	...
	AddInputDataAttributesToTable(InComputeGraphElementInputData);
}

void UPCGDataBinding::AddInputDataAttributesToTable(const FPCGDataCollection& InComputeGraphElementInputData)
{
	const UEnum* PCGKernelAttributeTypeEnum = StaticEnum<EPCGKernelAttributeType>();
	check(PCGKernelAttributeTypeEnum);

	int32 LargestExistingId = PCGComputeConstants::NUM_RESERVED_ATTRS - 1;

	for (TPair<FName /* Attribute name */, FPCGKernelAttributeIDAndType>& Pair : GlobalAttributeLookupTable)
	{
		LargestExistingId = FMath::Max(LargestExistingId, Pair.Value.Id);
	}
	...
	for (const FPCGTaggedData& Data : InComputeGraphElementInputData.TaggedData)
	{
		...
		for (int Index = 0; Index < AttributeNames.Num(); ++Index)
		{
			...
			FPCGKernelAttributeIDAndType* KernelAttribute = GlobalAttributeLookupTable.Find(AttributeName);

			if (!KernelAttribute)
			{
				GlobalAttributeLookupTable.Add(AttributeName, FPCGKernelAttributeIDAndType(++LargestExistingId, AttributeType));
			}
			else if (KernelAttribute->Type != AttributeType)
			{
				UE_LOG(LogPCG, Warning, TEXT("Encountered attribute '%s' multiple times with different types (%s vs %s). Attributes currently must have a single type in compute graphs."),
					*AttributeName.ToString(),
					*PCGKernelAttributeTypeEnum->GetNameStringByValue(static_cast<int64>(AttributeType)),
					*PCGKernelAttributeTypeEnum->GetNameStringByValue(static_cast<int64>(KernelAttribute->Type)));
			}
		}
	}
}
