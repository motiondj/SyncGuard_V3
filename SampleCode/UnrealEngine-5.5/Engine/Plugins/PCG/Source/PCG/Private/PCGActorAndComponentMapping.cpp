...
void FPCGActorAndComponentMapping::OnObjectChanged(UObject* InObject, const FActorPreviousData* InPreviousData, const UObject* InOriginatingChangeObject, bool bNoRefreshOwner)
{
	...
	auto Gather = [InObject, &RemovedTags, &MatchedKeys, InOriginatingChangeObject](TMap<FPCGSelectionKey, TSet<UPCGComponent*>> InMap, TSet<UPCGComponent*>& OutSet)
	{
		for (auto& It : InMap)
		{
			if (It.Key.IsMatching(InObject, RemovedTags, It.Value, &OutSet) || It.Key.IsMatching(InOriginatingChangeObject, RemovedTags, It.Value, &OutSet))
			{
