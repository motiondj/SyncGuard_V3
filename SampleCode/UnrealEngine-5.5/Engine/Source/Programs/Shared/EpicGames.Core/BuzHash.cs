...
#pragma warning restore CA1045 // Do not pass types by reference
		...
		/// </summary>
		/// <param name="span">Memory to find chunk points for</param>
		/// <param name="minSize">Minimum size for each chunk</param>
		/// <param name="maxSize">Maximum size for each chunk</param>
		/// <param name="targetSize">Target size for each chunk</param>
		/// <returns>List of chunk offsets and hashes</returns>
		public static List<(int, uint)> FindSplitPoints(ReadOnlySpan<byte> span, int minSize, int maxSize, int targetSize)
		{
		...
		/// </summary>
		/// <param name="span">Memory to find chunk points for</param>
		/// <param name="windowSize">Size of the hash window</param>
		/// <param name="threshold">Threshold below which to add a chunk point</param>
		/// <returns>List of chunk offsets and hashes</returns>
		public static unsafe List<(int, uint)> FindCandidateSplitPoints(ReadOnlySpan<byte> span, int windowSize, uint threshold)
		{
			...
			ReadOnlySpan<byte> prev = span.Slice(0, span.Length - windowSize);
			ReadOnlySpan<byte> next = span.Slice(windowSize);
