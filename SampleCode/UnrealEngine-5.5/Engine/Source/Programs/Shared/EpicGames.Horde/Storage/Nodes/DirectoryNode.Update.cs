...
namespace EpicGames.Horde.Storage.Nodes
{
	...
	public static class DirectoryNodeUpdateExtensions
	{
		...
		public static async Task AddFilesAsync(this DirectoryNode directoryNode, DirectoryReference baseDir, IEnumerable<FileInfo> files, IBlobWriter writer, ChunkingOptions? options = null, IProgress<IUpdateStats>? progress = null, CancellationToken cancellationToken = default)
		{
			...
			using (IEnumerator<FileInfo> fileEnumerator = files.GetEnumerator())
			{
				...
				for (bool moreData = fileEnumerator.MoveNext(); moreData;)
				{
					...
					{
						await CreateLeafChunkNodesAsync(writer, batch, leafChunkedFiles, 0, batch.Count, updateStats, options, cancellationToken);
					}
					else
					{
		...
		static async ValueTask CreateLeafChunkNodesAsync(IBlobWriter writer, IReadOnlyList<FileInfo> files, LeafChunkedData[] leafChunks, int start, int count, UpdateStats? updateStats, ChunkingOptions options, CancellationToken cancellationToken)
		{
			...
			{
				ContentChunker chunker = new ParallelBuzHashChunker(pipeline, buffer, options.LeafOptions);
				_ = pipeline.AddTask(ctx => WriteInputsAsync(files, start, count, chunker.SourceWriter, ctx));
				_ = pipeline.AddTask(ctx => HandleOutputsAsync(chunker.OutputReader, writerFork, leafChunks, updateStats, ctx));
		...
		static async Task WriteInputsAsync(IReadOnlyList<FileInfo> files, int start, int count, ChannelWriter<ChunkerSource> sourceWriter, CancellationToken cancellationToken)
		{
			for (int idx = start; idx < start + count; idx++)
			{
				FileChunkerSource source = new FileChunkerSource(files[idx], idx);
				await sourceWriter.WriteAsync(source, cancellationToken);
			}
			sourceWriter.Complete();
		}

		static async Task HandleOutputsAsync(ChannelReader<ChunkerOutput> outputReader, IBlobWriter writerFork, LeafChunkedData[] leafChunks, UpdateStats? updateStats, CancellationToken cancellationToken)
		{
			while (await outputReader.WaitToReadAsync(cancellationToken))
			{
				ChunkerOutput? output;
				if (outputReader.TryRead(out output))
				{
					int idx = (int)output.UserData!;
					leafChunks[idx] = await LeafChunkedDataNode.CreateFromChunkerOutputAsync(writerFork, output, updateStats, cancellationToken);
				}
			}
		}
