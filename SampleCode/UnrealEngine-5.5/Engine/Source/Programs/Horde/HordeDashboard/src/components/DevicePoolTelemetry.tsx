...
type DeviceProblem = { deviceId: string, deviceName: string, poolName: string, problems: number, problemsDesc: string, latestProblem?: DeviceTelemetry, reservable: boolean };
...
class PoolTelemetryHandler {
   ...
   getProblemDevices() {
      ...
      this.deviceTelemetry.forEach((telemetry, deviceId) => {
         ...
         const poolName = this.pools.get(device.poolId)!.name;
         const ratePercent = Math.floor(count / telemetry.length * 100);
         const reservable = device.enabled && !device.maintenanceTime;

         platforms.get(platform.id)!.push({ deviceId: deviceId, deviceName: device.name, poolName: poolName,problems: count, problemsDesc: `${count} / ${telemetry.length} (${ratePercent}%)`, latestProblem: latest, reservable: reservable });
...
class PoolTelemetryGraph {
   ...
   draw(container: HTMLDivElement, poolId: string, platformId: string, width: number, height: number) {
      ...
      avgLoad.append("rect")
         ...
         .style("stroke", function (d: any) { return modeColors.text })
...
const SummaryModal: React.FC<{ handler: PoolTelemetryHandler, selectedPlatform?: GetDevicePlatformResponse, onClose: () => void }> = ({ handler, selectedPlatform, onClose }) => {
   ...
   let platforms = Array.from(handler.platforms.values()).filter((p) => p.id === selectedPlatform?.id);
   ...
   const renderPlatform = (platform: GetDevicePlatformResponse) => {
      ...
      platformProblems = platformProblems.filter(p => p.problems > 8 && p.reservable).sort((a, b) => b.problems > a.problems ? 1 : -1);

      const problemColumns: IColumn[] = [
         { key: 'column1', name: 'Problem Devices', minWidth: 60, maxWidth: 150 },
         { key: 'column2', name: 'Pool', fieldName: 'poolName', minWidth: 60, maxWidth: 80 },
         { key: 'column3', name: 'Issues/Reservations', fieldName: 'problemsDesc', minWidth: 100, maxWidth: 150 },
         { key: 'column4', name: 'Latest Issue', minWidth: 300, maxWidth: 640 },
      ];
...
export const DevicePoolGraph: React.FC<{ poolIn?: string, platformIn?: string }> = observer(({ poolIn, platformIn }) => {
   ...
   if (!state.pool || !state.platform) {
      ...
      const pool = pools.find((e) => e.id == poolIn) ?? pools[0];
      const platform = platforms.find((e) => e.id == platformIn) ?? platforms[0];
         ...
         <Stack horizontal verticalAlign="start" style={{ width: 1412, paddingBottom: 18 }} tokens={{ childrenGap: 32 }}>
            ...
            <Dropdown
               ...
               onChange={(event, option) => {
                  ...
                  setState({ ...state, pool: option!.data, });
               }}
               style={{ width: 120 }}
            />
         </Stack>
      ...
      <Modal className={hordeClasses.modal} isOpen={true} isBlocking={true} topOffsetFixed={true} styles={{ main: { padding: 8, width: 1560, backgroundColor: modeColors.background, hasBeenOpened: false, top: "64px", position: "absolute", height: "760px" } }} onDismiss={() => { if (onClose) { onClose() } }}>
         <Stack>
            ...
            <Stack style={{ paddingLeft: 24 }}>
               <DevicePoolGraph poolIn={poolsIn?.find((e) => true)} platformIn={platformsIn?.find((e) => true)} />
            </Stack>
         </Stack>
      </Modal>
   </Stack>
