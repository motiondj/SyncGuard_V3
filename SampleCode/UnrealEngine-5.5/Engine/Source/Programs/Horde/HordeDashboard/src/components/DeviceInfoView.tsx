...
const RateBar = (rate: number, width: number, height: number): JSX.Element => {
   const colors = dashboard.getStatusColors();
   return (
      <Stack horizontal>
         <div style={{ backgroundColor: colors.get(StatusColor.Success), width: width, height: height, verticalAlign: 'middle', borderRadius: '6px', marginRight: '3px'}}>
            <span
               style={{
                  width: `${rate}%`, height: '100%',
                  backgroundColor: colors.get(StatusColor.Failure),
                  display: 'inline-block', borderRadius: 'inherit'
               }} />
         </div>
         <Label>{rate}% Failure Rate</Label>
      </Stack>
   );
}
...
export const DeviceInfoModal: React.FC<{ handler: DeviceHandler, deviceIn?: GetDeviceResponse | undefined, onEdit: (device: GetDeviceResponse) => void, onClose: () => void }> = observer(({ handler, deviceIn, onEdit, onClose }) => {
   ...
   let failureRate = jobState.telemetryItems.length == 0 ? 0 : Math.floor(jobState.telemetryItems.filter(item => !!item.job.problemTimeUtc).length / jobState.telemetryItems.length * 100);
               ...
               <Stack>
                  <Stack tokens={{ childrenGap: 12 }}>
                     ...
                     <Stack horizontal style={{ paddingTop: 24 }}>
                        ...
                        <Stack style={{ paddingTop: 8, paddingRight: 24 }}>
                           <DefaultButton text="Audit" onClick={() => { navigate(`/audit/device/${deviceIn.id}`) }} />
                        </Stack>
                     </Stack>
                  </Stack>
               </Stack>
               ...
               <Stack>
                  <Stack styles={{ root: { paddingLeft: 4, paddingRight: 0, paddingBottom: 4 } }}>
                     <Stack>
                        ...
                        {!!failureRate && RateBar(failureRate, 70, 15)}
                     </Stack>
