...
export const JobSearchSimpleModal: React.FC<{ streamIdIn: string, changeListIn?: string, jobTab?: GetJobsTabResponse | undefined, autoloadIn?: boolean, onClose: () => void }> = observer(({ streamIdIn, jobTab, changeListIn, onClose, autoloadIn }) => {
   ...
   const [searchState, setSearchState] = useState<{ items: JobItem[], groups?: IGroup[], templates: Set<string>, name?: string, userId?: string, global?: boolean, streamIdOverride?: string, preflights: boolean, preflightOnly?: boolean, minDate?: Date, maxDate?: Date, minCL?: string, maxCL?: string, querying: boolean, containsStep?: string, containsParameter?: string }>({ items: [], querying: false, preflights: false, minCL: changeListIn, maxCL: changeListIn, templates: new Set(jobTab?.templates ?? []) });
   ...
   Array.from(sorted.keys()).sort((a, b) => a < b ? -1 : 1).forEach(cat => {
      ...
      const subItems: IContextualMenuItem[] = templates.sort((a, b) => a.name < b.name ? -1 : 1).map(t => {
         ...
         return {
            ...
            text: t.name,
            data: t,
            onRender: (props: IContextualMenuItemProps) => {
   ...
   templateOptions.push({
      key: `show_all_templates`, text: "Clear Selection", onClick: (ev) => {
         ev?.stopPropagation();
         ev?.preventDefault();
   ...
   templateOptions.push({
      key: `show_all_templates`, text: "All Templates", onClick: (ev) => {
         ev?.stopPropagation();
         ev?.preventDefault();
   ...
   const ctemplates = templates.filter(t => searchState.templates.size === 0 || searchState.templates.has(t.id));
      ...
      try {
         ...
         if (stepName && !jobIds.length) {
            ...
            const query: JobQuery = {
               ...
               name: name,
               filter: "id,labels,defaultLabel,name,change,createTime,streamId,preflightChange,startedByUserInfo,arguments",
               minChange: minCL,
               maxChange: maxCL,
               template: (!searchState.global && ctemplates.length && !searchState.containsStep) ? ctemplates.map(t => t.id) : undefined,
            ...
            <Stack horizontal tokens={{ childrenGap: 48 }} >
               ...
               <Stack>
                  <Stack tokens={{ childrenGap: 12 }}>
                     ...
                     <Stack horizontal style={{ paddingTop: 24 }}>
                        ...
                        <Stack style={{ paddingTop: 8, paddingRight: 24 }}>
                           <DefaultButton disabled={searchState.querying} text="Clear" onClick={() => { setSearchState({ items: [], groups: [], querying: false, preflights: false, userId: searchState.userId, templates: new Set() }) }} />
                        </Stack>
                     </Stack>
                  </Stack>
               </Stack>
               ...
               <Stack>
                  <Stack styles={{ root: { paddingLeft: 4, paddingRight: 0, paddingBottom: 4 } }}>
                     ...
                     {!!searchState.items.length && <Stack>
                        <div style={{ overflowY: 'auto', overflowX: 'hidden', height: "670px" }} data-is-scrollable={true} onScroll={() => { controller.setState(undefined, true) }}>
