...
namespace HordeServer.Notifications
{
	...
	public sealed class NotificationService : IHostedService, INotificationService, IAsyncDisposable
	{
		...
		/// <inheritdoc/>
		public async Task<bool> UpdateSubscriptionsAsync(NotificationTriggerId triggerId, ClaimsPrincipal user, bool? email, bool? slack, CancellationToken cancellationToken)
		{
		...
		/// <inheritdoc/>
		public async Task<INotificationSubscription?> GetSubscriptionsAsync(NotificationTriggerId triggerId, ClaimsPrincipal user, CancellationToken cancellationToken)
		{
			UserId? userId = user.GetUserId();
			if (userId == null)
			{
		...
		/// <returns></returns>
		private async Task<INotificationTrigger?> GetNotificationTriggerAsync(NotificationTriggerId? triggerId, bool fireTrigger, CancellationToken cancellationToken)
		{
			if (triggerId == null)
			{
				return null;
		...
		private async Task<IReadOnlyList<IUser>> GetUsersToNotifyAsync(EventRecord? eventRecord, NotificationTriggerId? notificationTriggerId, bool fireTrigger, CancellationToken cancellationToken)
		{
		...
		private async Task SendAllLabelNotificationsAsync(IJob job, IReadOnlyList<(LabelState State, LabelOutcome Outcome)> oldLabelStates, IReadOnlyList<(LabelState, LabelOutcome)> newLabelStates, CancellationToken cancellationToken)
		{
			...
			for (int labelIdx = 0; labelIdx < graph.Labels.Count; ++labelIdx)
			{
				...
				if (oldLabel != newLabel)
				{
					...
					if (job.LabelIdxToTriggerId.TryGetValue(labelIdx, out NotificationTriggerId newTriggerId))
					{
		...
		private void SendLabelUpdateNotifications(IJob job, IGraph graph, IReadOnlyDictionary<NodeRef, IJobStep> stepForNode, ILabel label, int labelIdx, LabelOutcome outcome, IReadOnlyList<IUser> slackUsers)
		{
			...
			if (outcome != LabelOutcome.Success)
			{
				foreach (NodeRef includedNodeRef in label.IncludedNodes)
				{
					...
					stepData.Add((includedNode.Name, includedStep.Outcome, new Uri($"{_serverInfo.DashboardUrl}job/{job.Id}?step={includedStep.Id}")));
				}
			}
