...
namespace HordeServer.Telemetry.Metrics
{
	class MetricCollection : IMetricCollection, IHostedService, IAsyncDisposable
	{
		...
		public async Task FlushAsync(CancellationToken cancellationToken)
		{
			...
			Dictionary<SampleKey, MetricConfig> tbConstraint = new Dictionary<SampleKey, MetricConfig>();
			...
			try
			{
				foreach ((SampleKey sampleKey, List<double> sampleValues) in samples)
				{
					...
					if (_analyticsConfig.CurrentValue.TryGetTelemetryStore(sampleKey.Store, out TelemetryStoreConfig? telemetryStoreConfig) && telemetryStoreConfig.TryGetMetric(sampleKey.Metric, out metricConfig))
					{
						...
						if (metricConfig.TopN > 0 || metricConfig.BottomN > 0)
						{
							tbConstraint.Add(sampleKey, metricConfig);
						}
					}
				}
			}
			...
			// Constrain to TopN or BottomN
			/*
			HashSet<ObjectId> removeIds = new HashSet<ObjectId>();			
			if (tbConstraint.Count > 0)
			{
				foreach ((SampleKey sampleKey, MetricConfig metricConfig) in tbConstraint)
				{
					FilterDefinition < MetricDocument > filter = FilterDefinition<MetricDocument>.Empty;
					filter &= Builders<MetricDocument>.Filter.Eq(x => x.TelemetryStoreId, sampleKey.Store);
					filter &= Builders<MetricDocument>.Filter.Eq(x => x.Time, sampleKey.Time);
					filter &= Builders<MetricDocument>.Filter.Eq(x => x.MetricId, sampleKey.Metric);

					List<MetricDocument> results = await _metrics.Find(filter).SortByDescending(x => x.Value).ToListAsync(cancellationToken);

					if (results.Count <= metricConfig.TopN && results.Count <= metricConfig.BottomN)
					{
						continue;
					}
					...
					{
						for (int i = metricConfig.TopN; i < results.Count; i++)
						{
							removeIds.Add(results[i].Id);
						}
					}
					else
					{
						for (int i = 0; i < results.Count - metricConfig.BottomN; i++)
						{
							removeIds.Add(results[i].Id);
						}
					}
			...
			{
				FilterDefinition<MetricDocument> filter = FilterDefinition<MetricDocument>.Empty;
				filter &= Builders<MetricDocument>.Filter.In(x => x.Id, removeIds);
				await _metrics.DeleteManyAsync(filter, cancellationToken);
			}
			*/
		}
