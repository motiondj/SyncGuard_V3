...
#pragma warning restore CA2000
		...
		{
			List<ObjectId> importIds = new List<ObjectId>();
			if (imports.Count > 0)
			{
				foreach (BlobLocator import in imports)
				{
					ObjectId importId = await FindOrAddShadowBlobAsync(namespaceId, import, cancellationToken);
		...
		async Task<ObjectId> FindOrAddShadowBlobAsync(NamespaceId namespaceId, BlobLocator locator, CancellationToken cancellationToken = default)
		{
			...
			BlobInfo result = await _blobCollection.FindOneAndUpdateAsync(filter, update, new FindOneAndUpdateOptions<BlobInfo, BlobInfo> { IsUpsert = true, ReturnDocument = ReturnDocument.After }, cancellationToken);
			return result.Id;
		}
