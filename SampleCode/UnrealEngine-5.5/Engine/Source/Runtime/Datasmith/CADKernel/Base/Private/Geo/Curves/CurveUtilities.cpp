...
namespace UE::CADKernel::CurveUtilities
{
	...
	{
		const TArray<FPoint>& Poles = Curve.GetPoles();
	...
	TArray<FVector> GetPoles(const UE::CADKernel::FCurve& Curve)
	{
		...
		case ECurve::Segment:
		{
			...
			Poles.Reserve(2);
			{
				const FPoint& Point = Segment.GetStartPoint();
				Poles.Emplace(Point.X, Point.Y, Point.Z);
			}
			{
				const FPoint& Point = Segment.GetEndPoint();
				Poles.Emplace(Point.X, Point.Y, Point.Z);
			}
		}
		break;
		...
		{
			GetPolylinePoints<FPoint, FCurvePoint>(static_cast<const TPolylineCurve<FPoint, FCurvePoint>&>(Curve), Poles);
		}

		case ECurve::Polyline2D:
		{
			GetPolylinePoints<FPoint2D, FCurvePoint2D>(static_cast<const TPolylineCurve<FPoint2D, FCurvePoint2D>&>(Curve), Poles);
		}
		break;
	...
	TArray<FVector2d> Get2DPolyline(const UE::CADKernel::FRestrictionCurve& Curve)
	{
		...
		const TArray<FPoint2D>& Point2Ds = Polyline.Get2DPoints();

		TArray<FVector2d> SurfacicPoints;
		SurfacicPoints.Reserve(Point2Ds.Num());

		for (const FPoint2D& Point2D : Point2Ds)
		{
			SurfacicPoints.Emplace(Point2D.U, Point2D.V);
		}
	...
	TArray<FVector> Get3DPolyline(const UE::CADKernel::FRestrictionCurve& Curve)
	{
		...
		const TArray<FPoint>& Point3Ds = Polyline.GetPoints();

		TArray<FVector> Points;
		Points.Reserve(Point3Ds.Num());

		for (const FPoint& Point : Point3Ds)
		{
	...
	int32 GetPoleCount(const UE::CADKernel::FCurve& Curve)
	{
		...
		case ECurve::Polyline3D:
		{
			return static_cast<const TPolylineCurve<FPoint, FCurvePoint>&>(Curve).GetPolylinePoints().Num();
		}

		case ECurve::Polyline2D:
		{
			return static_cast<const TPolylineCurve<FPoint2D, FCurvePoint2D>&>(Curve).GetPolylinePoints().Num();
		}
