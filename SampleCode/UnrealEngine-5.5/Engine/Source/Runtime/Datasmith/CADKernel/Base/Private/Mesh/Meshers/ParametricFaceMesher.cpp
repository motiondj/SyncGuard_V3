...
bool FParametricFaceMesher::MeshPlanarFace()
{
	...
	const TArray<TArray<FPoint2D>>& Loops2D = Grid.GetLoops2D(EGridSpace::Default2D);
	...
	for (int32 Index = 0; Index < Loops.Num(); ++Index)
	{
		...
		for (const FPoint2D& Point : PointList)
		{
			VertexList.Emplace(Point.U, Point.V);
		}
	...
	TArray<FPoint>& NodeCoordinates = FaceMesh.GetNodeCoordinates();
	...
	for (FGeneralPolygon2d& Polygon : PolygonsOut)
	{
		...
		for (const FVector2d& Vertex : Vertices)
		{
			FPoint Point;
			FVector3f Normal;
			CarrierSurface.EvaluatePointAndNormal(FPoint2D(Vertex.X, Vertex.Y), Point, Normal);
