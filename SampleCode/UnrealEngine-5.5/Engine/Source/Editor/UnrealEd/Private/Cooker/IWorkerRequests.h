...
public:
	 ...
	 * May be out of date after calling; do not assume a true return value means Requests are actually present or a
	 * false value means no Requests are present.
	 * Intended usage is for the Scheduler to be the only consumer of requests, and to use this value for periodic
	 * checking of whether there is any work that justifies the expense of taking the lock.
	 * In a single-consumer case, HasRequests will eventually correctly return true as long as the consumer is not
	 * consuming.
	 */
	 ...
	 * May be out of date after calling; do not assume the number of actual requests is any one of equal, greater than,
	 * or less than the returned value.
	 * Intended usage is for the Scheduler to be the only consumer of requests, and to use this value for rough
	 * reporting of periodic progress.
	 */
	virtual int32 GetNumExternalRequests() const = 0;
	/**
	 * If this Worker has any callbacks, dequeue them all into OutCallbacks and return EExternalRequestType::Callback;
	 * Callbacks take priority over cook requests.
	 * Otherwise, if there are any cook requests, dequeue them all into OutBuildRequests and return
	 * EExternalRequestType::Cook.
	...
	/**
	 * Move all callbacks into OutCallbacks, and all cook requests into OutCookRequests. This is used when canceling
	 * a cook session.
	 */
	virtual void DequeueAllExternal(TArray<FSchedulerCallback>& OutCallbacks,
		TArray<FFilePlatformRequest>& OutCookRequests) = 0;
	...
	virtual void GetInitializeConfigSettings(UCookOnTheFlyServer& COTFS, const FString& OutputDirectoryOverride,
		UE::Cook::FInitializeConfigSettings& Settings) = 0;
	virtual void GetBeginCookConfigSettings(UCookOnTheFlyServer& COTFS, FBeginCookContext& BeginContext,
		UE::Cook::FBeginCookConfigSettings& Settings) = 0;
