...
namespace UE::Private
{
	...
	EHostType::Type FindModuleDescriptor(const UPackage* ForNativePackage);
}
...
EHostType::Type UE::Private::FindModuleDescriptor(const UPackage* ForNativePackage)
{
	...
	const auto AddToCache = [](FName Name, EHostType::Type HostType)
	{
		...
		FScopeLock WriteLock(&DescriptorCacheLock);
		static bool bCheckedForHotReload = false;
		if(!bCheckedForHotReload)
		{
			bCheckedForHotReload = true;
			// Register the hot-reload callback - invalidating our cache when a module is compiled:
			if(IHotReloadModule::IsAvailable())
			{
				IHotReloadModule::Get().OnModuleCompilerStarted().AddStatic( &InvalidateCache );
			}
			
#if WITH_LIVE_CODING
			if (ILiveCodingModule* LiveCoding = FModuleManager::LoadModulePtr<ILiveCodingModule>(LIVE_CODING_MODULE_NAME))
			{
				LiveCoding->GetOnPatchCompleteDelegate().AddStatic(&InvalidateCache);
			}
#endif
		}
		DescriptorCache.Add(Name, HostType);
	};
...
FBuildTargetSet::EBuildTargetFlags FBuildTargetSet::GetSupportedTargetsForNativeClass(const UClass* NativeBase)
{
	...
	const EHostType::Type ModuleHostType = FindModuleDescriptor(Package);
